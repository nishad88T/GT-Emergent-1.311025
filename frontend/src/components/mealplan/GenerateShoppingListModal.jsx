...
import { Recipe, MealPlan } from "@/api/entities";
...
const aggregateIngredients = (recipes, ingredientMaps) => {
    ...
    recipes.forEach(({ recipe, servings }) => {
        ...
        recipe.ingredients.forEach(ingredient => {
            const category = categorizeIngredient(ingredient, ingredientMaps); // This 'category' is the specific one (e.g., 'Produce', 'Dairy')
            ...
            aggregated[displayCategory].push({
                ...
                // servingMultiplier, // Removed as it's not used in the output
                recipeTitle: recipe.title,
                aisle_order: aisleOrder // This is the specific aisle order for the ingredient
            });
        });
    });
...
const CategorySection = ({ category, items, selectedItems, onItemToggle, onCategoryToggle }) => {
        ...
        try {
                ...
                items.forEach(item => {
                    if (selectedItems.has(item.id)) {
                        ...
                        selectedIngredientsList.push({
                            ...
                            category: specificCategory, // Now using specific category like 'Produce', 'Dairy'
                            aisle_order: CATEGORY_TO_AISLE_ORDER[specificCategory] || 120, // Use aisle order from specific category
            ...
            const newItems = selectedIngredientsList
                ...
                .map((ing, index) => ({
                    id: Date.now() + index,
                    name: ing.name,
                    category: ing.category, // Now using specific category from selectedIngredientsList
                    estimated_price: 0,
                    priority: 'medium',
                    checked: false,
                    from_meal_plan: true,
