                    ...
                    {!hasInflationData && !hasShrinkflationData ? (
                        ...
                        <Tabs defaultValue="inflation" className="w-full">
                            <TabsList className="grid w-full grid-cols-2 h-auto">
                                <TabsTrigger value="inflation" className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm px-2 sm:px-4 py-2">
                                    <ArrowUp className="w-3 h-3 sm:w-4 sm:h-4" />
                                    <span className="hidden sm:inline">Price Inflation</span>
                                    <span className="sm:hidden">Inflation</span>
                                    <span className="ml-1">({inflationData.inflation.length})</span>
                                </TabsTrigger>
                                <TabsTrigger value="shrinkflation" className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm px-2 sm:px-4 py-2">
                                    <Package className="w-3 h-3 sm:w-4 sm:h-4" />
                                    <span className="hidden sm:inline">Shrinkflation</span>
                                    <span className="sm:hidden">Shrink</span>
                                    <span className="ml-1">({inflationData.shrinkflation.length})</span>
                                </TabsTrigger>
                            </TabsList>
                            ...
                            <TabsContent value="shrinkflation" className="space-y-4 mt-4">
                                <Alert className="border-amber-200 bg-amber-50">
                                    <Info className="h-4 w-4 text-amber-600" />
                                    <AlertDescription className="text-xs sm:text-sm text-amber-800">
                                        <strong>Note:</strong> Shrinkflation analysis requires pack size and unit information (e.g., "500g", "1L") to be present on your receipts. 
                                        If your receipts don't include this data, accurate shrinkflation detection may not be possible for those items.
                                    </AlertDescription>
                                </Alert>
                                
                                {hasShrinkflationData ? (
                                    <div className="space-y-4">
                                        <p className="text-xs sm:text-sm text-slate-600">
                                            Products with reduced pack sizes at similar prices, comparing "{comparisonPeriods?.current?.label || 'current'}" to "{comparisonPeriods?.comparison?.label || 'previous'}" period.
                                        </p>
                                        <div className="overflow-x-auto -mx-2 sm:mx-0">
                                            <Table>
                                                <TableHeader>
                                                    <TableRow>
                                                        <TableHead className="text-xs sm:text-sm">Item</TableHead>
                                                        <TableHead className="text-xs sm:text-sm hidden md:table-cell">Comparison Size</TableHead>
                                                        <TableHead className="text-xs sm:text-sm hidden md:table-cell">Current Size</TableHead>
                                                        <TableHead className="text-xs sm:text-sm">Change</TableHead>
                                                        <TableHead className="text-xs sm:text-sm">Actions</TableHead>
                                                    </TableRow>
                                                </TableHeader>
                                                <TableBody>
                                                    {inflationData.shrinkflation.map((item, index) => (
                                                        <TableRow key={index}>
                                                            <TableCell className="font-medium text-xs sm:text-sm">
                                                                <div>
                                                                    <div>{item.product}</div>
                                                                    <div className="md:hidden text-xs text-slate-500 mt-1">
                                                                        {item.packSizeLastMonth.toFixed(1)}{item.unit} → {item.packSizeThisMonth.toFixed(1)}{item.unit}
                                                                    </div>
                                                                </div>
                                                            </TableCell>
                                                            <TableCell className="hidden md:table-cell text-xs sm:text-sm">{item.packSizeLastMonth.toFixed(1)} {item.unit}</TableCell>
                                                            <TableCell className="hidden md:table-cell text-xs sm:text-sm">{item.packSizeThisMonth.toFixed(1)} {item.unit}</TableCell>
                                                            <TableCell>
                                                                <Badge className="bg-orange-100 text-orange-700 text-xs">
                                                                    <ArrowDown className="w-2 h-2 sm:w-3 sm:h-3 mr-1" />
                                                                    {(item.packSizeChange * 100).toFixed(1)}%
                                                                </Badge>
                                                            </TableCell>
                                                            <TableCell>
                                                                <Button size="sm" variant="outline" onClick={() => handleViewClick('shrinkflation', item)} className="text-xs px-2 py-1">
                                                                    <Eye className="w-3 h-3 sm:w-4 sm:h-4 sm:mr-2" />
                                                                    <span className="hidden sm:inline">View</span>
                                                                </Button>
                                                            </TableCell>
                                    ...
                                    <Alert>
                                        <AlertTriangle className="h-4 w-4" />
                                        <AlertDescription className="text-xs sm:text-sm">
                                            No shrinkflation detected between the selected periods.
                                        </AlertDescription>
                                    </Alert>
                                )}
                            </TabsContent>
