    ...
    const volatilityData = useMemo(() => {
        ...
        relevantReceipts.forEach(receipt => {
            ...
            (receipt.items || []).forEach(item => {
                ...
                // CRITICAL FIX: Normalize price by quantity to get price per unit
                const totalPrice = parseFloat(item.total_price) || 0;
                const quantity = Math.max(1, parseInt(item.quantity) || 1);
                const pricePerUnit = totalPrice / quantity; // This is the key fix!

                itemPurchases[key].purchases.push({
                    date: purchaseDate,
                    pricePerUnit: pricePerUnit, // Normalized price for comparison
                    totalPrice: totalPrice, // Original total for display
        ...
        <div className={className}>
            <Card className="border-none shadow-lg bg-white/80 backdrop-blur-sm">
                <CardHeader>
                    ...
                    <CardDescription>
                        {demoData ? "Demo: Sample items with price variations" : "Items with significant price variations across your purchases (normalized by quantity)"}
                    </CardDescription>
                </CardHeader>
                <CardContent>
